...
namespace Jakar.Api
{
	public static class Requests
	{
		...
		{
			public static async Task<string> Get( string baseUrl, CancellationToken token, params string[] parameters ) =>
				await Get(new Uri(baseUrl, UriKind.Absolute), 10000, token, parameters).ConfigureAwait(true);

			public static async Task<string> Get( string baseUrl, int timeout, CancellationToken token, params string[] parameters ) =>
				await Get(new Uri(baseUrl, UriKind.Absolute), timeout, token, parameters).ConfigureAwait(true);
