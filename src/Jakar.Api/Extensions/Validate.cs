...
namespace Jakar.Api.Extensions
{
	public static class Validate
	{
		...
		public static string FormatNumber<T>( this T number, int maxDecimals = 4 ) => Regex.Replace(string.Format(Language.Current.CultureInfo, $"{{0:n{maxDecimals}}}", number), $"[{Language.Current.CultureInfo.NumberFormat.NumberDecimalSeparator}]?0+$", "");
		...
		public static IPAddress? ParseIPv4( string? ipString ) => IPAddress.TryParse(ipString!, out IPAddress address) ? address : null;


		public static bool IsWebAddress( string? addressString )
		{
		...
		public static Uri? ParseWebAddress( string? addressString ) => Uri.TryCreate(addressString, UriKind.Absolute, out Uri uriResult) ? uriResult : null;
		...
		public static bool ValidateEmail( this string email )
		{
			...
			return email.Contains("@", StringComparison.OrdinalIgnoreCase) && email.Contains(".", StringComparison.OrdinalIgnoreCase) && ( !email.Contains(",", StringComparison.OrdinalIgnoreCase) || !email.Contains("~", StringComparison.OrdinalIgnoreCase) );
		}
		...
		public static string ThrowIfNull( string arg, string name, [CallerMemberName] string caller = "" ) => string.IsNullOrWhiteSpace(arg) ? throw new ArgumentNullException(name, caller) : arg;
	}
}